...
monitor の高CPU検出は `--watch` / `--daemon` モードで連続検知方式を採用している。
高CPU（80%超）プロセスを検出しても初回は警告のみで終了せず、次の監視サイクルでも
高CPUが継続している場合にのみ強制終了する。
一時的なCPUスパイク（コンパイル、コード解析等）での誤killを防止する。
単発実行（オプションなし）では従来通り即座に強制終了する。
